...
import engine.systems.global_.popup as popup_feature
import engine.factories.dialog as dialog_entities
...
def process_interaction(scene: "SettingsScene", logic_data: dict):
    match logic_data["interaction_name"]:
        case "back_to_menu":
            error = shared.utils.save.apply_settings(scene.save, scene.window)
            if error:
                _, message = error
                ...
                return
            shared.utils.save.save_settings(scene.save)
            return "menu"
