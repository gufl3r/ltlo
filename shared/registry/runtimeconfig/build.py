...
def build(cfg: dict):
    ...
    def numeric_stepper_distances(entities: list) -> dict:
        ...
        for i, entity in enumerate(entities):
            ...
            if "decrease" in tags and "button_rect" in tags:
                if decrease_idx is None:
                    decrease_idx = i

            elif "increase" in tags and "button_rect" in tags:
                if increase_idx is None:
                    increase_idx = i

            elif (
                "numeric_stepper" in tags
                and "button_label" in tags
                and "decrease" not in tags
                and "increase" not in tags
            ):
