...
import sys
import shared.registry.runtimeconfig.build as build
...
def load(file_path="shared/registry/storages/runtimeconfig.json"):
    global BASE_RESOLUTION, UI_OFFSETS, _LOADED

    if _LOADED:
        return

    with open(engine.utils.path.resource_path(file_path), "r") as f:
        current_cfg = json.load(f)
    _validate(current_cfg)
    if not getattr(sys, "frozen", False): # dev
        cfg = build.build(current_cfg)
        _validate(cfg)
        with open(engine.utils.path.resource_path(file_path), "w") as f:
            json.dump(cfg, f)
    else:
        cfg = current_cfg
