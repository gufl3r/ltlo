...
class Scene:
    ...
    def __init__(self, window: Window, save: dict) -> None:
        ...
        self._inject_system_entities()

    def _inject_system_entities(self):
        esc_listener = generic_entities.keyboard_listener(
            name="sys_esc_listener",
            key_symbol=key.ESCAPE,
            interaction_name="request_pause", 
            duration=-1
        )

        self._entities.append(esc_listener)

    def register_handlers(self):
        @self.window.event
        def on_close():
            self._input_queue.append({"name": "close"})
    ...
    def play_cutscene(self, video_asset: pyglet.media.Source):
        ...
        result = self.loop_subscene(VideoSubScene, {"video_asset": video_asset})
        if result == "exit_to_menu":
            self._logic_queue.append({"name": "interaction", "data": {"interaction_name": "exit_to_menu"}})
        elif result == "video_end":
            self._logic_queue.append({"name": "interaction", "data": {"interaction_name": "video_end"}})
    ...
    def pause(self):
        ...
        for entity in self._entities:
            if entity.drawable.width == 0:
                continue
            new_entity = dataclasses.replace(entity, ticks_left=-1)

            pause_state_index = -1
            for i, state in enumerate(new_entity.states):
                if state.name == "_animation_pause_frame_index":
                    pause_state_index = i
                    break

            if pause_state_index != -1:
                current_frame = entity.drawable.frame_index
                
                new_states = new_entity.states.copy()
                
                new_states[pause_state_index] = scene_types.State(
                    "_animation_pause_frame_index",
                    {"value": current_frame}
                )

                new_entity = dataclasses.replace(new_entity, states=new_states)

            background_snapshot.append(new_entity)

        result = self.loop_subscene(PauseSubScene, {"background": background_snapshot})
        if result == "exit_to_menu":
            self._logic_queue.append({"name": "interaction", "data": {"interaction_name": "exit_to_menu"}})
        elif result == "resume_game":
            self._logic_queue.append({"name": "interaction", "data": {"interaction_name": "resume_game"}})

    def relative_position(self, x: int, y: int) -> tuple[int, int]:
        return engine.utils.conversions.convert_position(
            (x, y),
    ...
    def entities_by_tags(self, required: list[str] | None = None, match: list[str] | None = None):
        ...
        for entity in self._entities:
            if required and not all(tag in entity.tags for tag in required):
                continue

            if match and not any(tag in entity.tags for tag in match):
                continue
                ...
                case "mouse_press":
                    for entity in reversed(self._entities):
                        if (
                            ...
                            engine.utils.detections.point_inside_area(
                                event["data"]["position"],
    ...
    def _tick_audio(self):
        if self.audio_player.source and self.audio_player.time > self.audio_player.source.duration:
            ...
            self._after_audio(self.audio_player)
    ...
    def _after_audio(self, player: pyglet.media.Player) -> None:
        self._logic_queue.append({"name":"after_audio","data":{"source":player.source}})
        if player.loop:
            player.seek(0)
        else:
            player.next_source()
    ...
    def _update_entities(self) -> None:
        ...
        for i, entity in enumerate(self._entities):
            ...
            new_entity = dataclasses.replace(
                ...
                id_=engine.utils.ids.generate_id() if entity.ticks_alive == 0 else entity.id_,
